...
<script type="text/javascript">
	function validate() {
		...
		if (checkValueError("_id", 9, "editERROR", "提交失败: 首页主键id不能为空,不能超过9个字符.")) {
			return false;
		}
		if (checkValueError("_homePageID", 9, "editERROR",
		...
		if (checkValueError("_musicTitle", 100, "editERROR",
				"提交失败: 音乐标题不能为空,不能超过100个字符.")) {
			return false;
		}
		if (checkValueError("_musicAuthor", 100, "editERROR",
				"提交失败: 音乐作者不能为空,不能超过100个字符.")) {
			return false;
		}
		if (checkValueError("_musicImage", 100, "editERROR",
				"提交失败: 音乐图标uri不能为空,不能超过100个字符.")) {
			return false;
		}
		if (checkValueError("_musicURL", 100, "editERROR",
...
<body style="margin: 5px;">
	...
	<div class="easyui-panel" title="编辑首页" style="width: auto">
		<div style="padding: 10px 20px 10px 20px">
			<s:form id="editform" action="home_edit" method="post">
				<table cellpadding="1" class="table table-bordered">
					<!-- hidden隐藏主键_id,更新的时候传回后台用到 -->
					<s:hidden id="_id" name="home._id"></s:hidden>
					<tr>
						<td></td>
					...
					<tr>
						...
						<td><s:textfield id="_likeCount" name="home._likeCount"
								class="comments" readonly="true" cols="20" /></td>
					</tr>
					<tr>
						<td>音乐标题:</td>
