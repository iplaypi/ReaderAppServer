...
<script type="text/javascript">
	function validate() {
		...
		if (checkValueError("_picturePageID", 9, "addERROR",
				"提交失败: 图片编号不能为空,不能超过9个字符.")) {
			return false;
		}
		if (!isNum("_picturePageID", 1, 9)) {
			setValue("addERROR", "提交失败: 图片编号必须是1-9位的数字.");
			document.getElementById("_picturePageID").focus();
			return false;
		}
...
<body style="margin: 5px;">
	...
	<div class="easyui-panel" title="添加图片" style="width: auto">
		<div style="padding: 10px 20px 10px 20px">
			<s:form id="addform" action="picture_add" method="post">
				<table cellpadding="1" class="table table-bordered">
					<tr>
						<td><p id="addSUCCESS" style="color: blue; font-size: 1.5em">${addSUCCESS }</p></td>
					...
					<tr>
						<td></td>
						<td><p>
								<font color="blue" size="4">图片编号必须从最大的开始递增,不能随意指定.</font>
							</p></td>
					</tr>
					<tr>
						<td>图片编号:</td>
						<td><s:textfield id="_picturePageID"
								name="picture._picturePageID" class="comments" /></td>
					</tr>
					<tr>
						<td>图片标题:</td>
					...
					<tr>
						...
						<td><a href="picture_addUI.action" class="easyui-linkbutton">重置</a></td>
					</tr>
				</table>
			</s:form>
		</div>
